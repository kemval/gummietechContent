...
def generate(prompt: str, api_key: str, model: str,
    ...
    # before the JSON; under load that sometimes eats the whole budget and
    # Groq rejects the truncated result with json_validate_failed. "low"
    # leaves more room for the actual output. Valid values are low/medium/
    # high only — "none" 400s on these models, unlike qwen3.
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if resp.status_code == 400 and "json_validate_failed" in resp.text:
            # Groq's own docs call this a transient failure of best-effort
            # JSON mode and recommend retrying, not treating it as a bad
            # request - the model occasionally burns its completion budget
            # on hidden reasoning before finishing the JSON.
            if attempt == MAX_RETRIES:
                raise SystemExit(
                    f"Groq kept failing JSON validation after {MAX_RETRIES} "
                    "attempts. Try a smaller batch size, or set GROQ_MODEL "
                    "to openai/gpt-oss-120b (larger budget, fails less often).")
            print(f"  Groq json_validate_failed (attempt {attempt}/{MAX_RETRIES}), retrying")
            time.sleep(delay)
            delay *= 2
            continue
